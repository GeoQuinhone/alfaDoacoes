    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-2xl font-bold text-gray-800">Editar Item</h1>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="descricao" id="descricao" rows="3"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-green-500 focus:border-green-500">{{ old('descricao', $item->descricao) }}</textarea>
                        @error('descricao')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="categoria" id="categoria"
                            ...
                            <option value="alimentos"
                                {{ old('categoria', $item->categoria) == 'alimentos' ? 'selected' : '' }}>Alimentos</option>
                            <option value="roupas" {{ old('categoria', $item->categoria) == 'roupas' ? 'selected' : '' }}>
                                Roupas</option>
                            <option value="higiene" {{ old('categoria', $item->categoria) == 'higiene' ? 'selected' : '' }}>
                                Higiene Pessoal</option>
                            <option value="medicamentos"
                                {{ old('categoria', $item->categoria) == 'medicamentos' ? 'selected' : '' }}>Medicamentos
                            </option>
                            <option value="brinquedos"
                                {{ old('categoria', $item->categoria) == 'brinquedos' ? 'selected' : '' }}>Brinquedos
                            </option>
                            <option value="moveis" {{ old('categoria', $item->categoria) == 'moveis' ? 'selected' : '' }}>
                                Móveis</option>
                            <option value="eletronicos"
                                {{ old('categoria', $item->categoria) == 'eletronicos' ? 'selected' : '' }}>Eletrônicos
                            </option>
                            <option value="outros" {{ old('categoria', $item->categoria) == 'outros' ? 'selected' : '' }}>
                                Outros</option>
                        </select>
                        @error('categoria')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        ...
                        <select name="instituicao_id" id="instituicao_id"
                            ...
                            @foreach ($instituicoes as $instituicao)
                                <option value="{{ $instituicao->id }}"
                                    {{ old('instituicao_id', $item->instituicao_id) == $instituicao->id ? 'selected' : '' }}>
                                    {{ $instituicao->nome }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="quantidade_disponivel" id="quantidade_disponivel"
                            value="{{ old('quantidade_disponivel', $item->quantidade_disponivel) }}" min="0"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-green-500 focus:border-green-500"
                            required>
                    ...
                    <div>
                        ...
                        <input type="number" name="quantidade_necessaria" id="quantidade_necessaria"
                            value="{{ old('quantidade_necessaria', $item->quantidade_necessaria) }}" min="0"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-green-500 focus:border-green-500"
                            required>
                    ...
                    <div class="md:col-span-2">
                        <div class="flex items-center">
                            <input type="checkbox" name="urgente" id="urgente" value="1"
                                {{ old('urgente', $item->urgente) ? 'checked' : '' }}
